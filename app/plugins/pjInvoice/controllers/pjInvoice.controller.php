...
class pjInvoice extends pjInvoiceAppController
{
		...
		if (isset($_POST['invoice_post']))
		{
			if (isset($_FILES['y_logo']) && !empty($_FILES['y_logo']['tmp_name']))
			{
				...
				if ($pjImage->load($_FILES['y_logo']))
				{
					...
					pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjInvoice&action=pjActionIndex&err=PIN03&errTime=" . $time);
				}
			}
			$data = array();
			...
			pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjInvoice&action=pjActionIndex&err=PIN02&tab_id=" . $_POST['tab_id']);
		}
		...
		switch ($_POST['payment_method'])
		{
			case 'paypal':
				$this->set('params', array(
					...
					'return' => PJ_INSTALL_URL . 'admin.php?controller=pjInvoice&action=pjActionView&uuid=' . $arr['uuid'],
					'notify_url' => PJ_INSTALL_URL . 'admin.php?controller=pjInvoice&action=pjActionConfirmPaypal&cid=' . $arr['foreign_id']
				));
				break;
			case 'authorize':
				$this->set('params', array(
					...
					'x_receipt_link_url' => PJ_INSTALL_URL . 'admin.php?controller=pjInvoice&action=pjActionView&uuid=' . $arr['uuid'],
					'x_relay_url' => PJ_INSTALL_URL . 'admin.php?controller=pjInvoice&action=pjActionConfirmAuthorize&cid=' . $arr['foreign_id']
				));
				break;
		}
	...
	public function pjActionSend()
	{
		...
		if ($this->isXHR() && $this->isLoged() && $this->isInvoiceReady())
		{
			...
			if (isset($_POST['uuid']) && !empty($_POST['uuid']))
			{
				...
				$view_url = PJ_INSTALL_URL . 'admin.php?controller=pjInvoice&action=pjActionView&id=' . $_POST['id'] . '&uuid=' . $_POST['uuid'];
	...
	private function pjActionTokenizer($a, $locale_id=NULL)
	{
		...
		if (isset($a['items']) && is_array($a['items']) && !empty($a['items']))
		{
			...
			{
				$items .= '<tr>';
				$items .= sprintf('<td>%s<br>%s</td>', $item['name'], htmlspecialchars_decode($item['description']));
				if($a['o_use_qty_unit_price'] == 1)
				{
	...
	public function pjActionUpdate()
	{
		...
		if (isset($_POST['invoice_update']))
		{
			...
			if (empty($arr))
			{
				pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjInvoice&action=pjActionInvoices&err=PIN04");
			}
			...
			pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjInvoice&action=pjActionUpdate&id=".$_POST['id']."&err=PIN05");
		}
