...
(function ($, undefined) {
	...
	$(function () {
		...
		if ($frmCreateInvoice.length > 0) {
			$frmCreateInvoice.validate({
				rules: {
					"uuid": {
						required: true,
						remote: "admin.php?controller=pjInvoice&action=pjActionCheckUniqueId"
					},
					"status": "required"
				},
				messages:{
		...
		if ($frmUpdateInvoice.length > 0) {
			$frmUpdateInvoice.validate({
				rules: {
					"uuid": {
						required: true,
						remote: "admin.php?controller=pjInvoice&action=pjActionCheckUniqueId&id=" + $frmUpdateInvoice.find("input[name='id']").val()
					},
					"b_email": {
						email: true
					},
		...
		$(document).on("search", ".frm-filter", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjInvoice&action=pjActionGetInvoices", "created", "DESC", content.page, content.rowCount);
		...
		if ($("#grid").length > 0 && datagrid) {
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjInvoice&action=pjActionUpdate&id={:id}", title: "Edit"},
				          {type: "delete", url: "admin.php?controller=pjInvoice&action=pjActionDelete&id={:id}", title: "Delete"},
				          {type: "menu", url: "#", text: '', items:[
				                     {text: myLabel.view_invoice, target: "_blank", url: "admin.php?controller=pjInvoice&action=pjActionView&id={:uuid}&uuid={:order_id}"},
				                     {text: myLabel.print_invoice, target: "_blank", url: "admin.php?controller=pjInvoice&action=pjActionPrint&id={:uuid}&uuid={:order_id}"},
									 {text: myLabel.email_invoice, url: "admin.php?controller=pjInvoice&action=pjActionEmail&id={:uuid}&uuid={:order_id}", linkClass: 'plugin_invoice_email'}
                                ]
				          }
						 ],
				columns: [
				...
				dataUrl: "admin.php?controller=pjInvoice&action=pjActionGetInvoices",
				dataType: "json",
				fields: ['order_id', 'issue_date', 'due_date', 'created', 'status', 'paid_deposit'],
				paginator: {
					actions: [
					   {text: myLabel.delete_title, url: "admin.php?controller=pjInvoice&action=pjActionDeleteBulk", render: true, confirmation: myLabel.delete_body}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjInvoice&action=pjActionSaveInvoice&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($("#grid_items").length > 0 && datagrid) {
			...
			var $grid = $("#grid_items").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjInvoice&action=pjActionUpdateItem&id={:id}", title: "Edit"},
				          {type: "delete", url: "admin.php?controller=pjInvoice&action=pjActionDeleteItem&id={:id}", title: "Delete"}],
				columns: columns,
				dataUrl: "admin.php?controller=pjInvoice&action=pjActionGetItems&invoice_id=" + invoice_id,
				dataType: "json",
				fields: fields,
				paginator: false,
				saveUrl: "admin.php?controller=pjInvoice&action=pjActionSaveItem&id={:id}",
				select: false
			});
			...
			if (tmp !== undefined) {
				...
				window.setTimeout(function () {
					$grid.datagrid("load", "admin.php?controller=pjInvoice&action=pjActionGetItems", "id", "ASC", content.page, content.rowCount);
				}, 2000);
			}
		}
		...
		if ($dialogSendInvoice.length > 0 && dialog) {
			...
			buttons[myLabel.btn_send] = function () {
				...
				$btn.button('disable');
				$.post("admin.php?controller=pjInvoice&action=pjActionSend", $dialogSendInvoice.find("form").serialize()).always(function () {
					$this.dialog("close");
				}).always(function () {
					$btn.button('enable');
				});
			...
			$dialogSendInvoice.dialog({
				...
				open: function () {
					...
					$.get("admin.php?controller=pjInvoice&action=pjActionSend", {
						"id": $this.data("id"),
						"uuid": $this.data("uuid")
					}).done(function (data) {
		...
		if ($dialogDeleteLogo.length > 0 && dialog) {
			...
			buttons[myLabel.btn_delete] = function () {
				var $this = $(this);
				$.post("admin.php?controller=pjInvoice&action=pjActionDeleteLogo").done(function () {
		...
		if ($dialogAddItem.length > 0 && dialog) {
			...
			buttons[myLabel.btn_save] = function () {
				...
				if ($form.length > 0 && validate) {
					...
					if (validator.form()) {
						$.post("admin.php?controller=pjInvoice&action=pjActionAddItem", $form.serialize()).done(function (data) {
							...
							var content = $grid.datagrid("option", "content");
							$grid.datagrid("load", "admin.php?controller=pjInvoice&action=pjActionGetItems&invoice_id=" + invoice_id, "id", "ASC", content.page, content.rowCount);
						}).always(function () {
							$this.dialog("close");
						});
					}
			...
			$dialogAddItem.dialog({
				...
				open: function () {
					$dialogAddItem.html("");
					$.get("admin.php?controller=pjInvoice&action=pjActionAddItem", {
		...
		if ($dialogEditItem.length > 0 && dialog) {
			...
			buttons[myLabel.btn_update] = function () {
				...
				if ($form.length > 0 && validate) {
					var validator = $form.validate(itemOpts);
					if (validator.form()) {
						$.post("admin.php?controller=pjInvoice&action=pjActionEditItem", $form.serialize()).done(function (data) {
							if (data.total) {
								$("#total").val(data.total.toFixed(2));
								$("#subtotal").val(data.total.toFixed(2));
								$("#paid_deposit").val(data.total.toFixed(2));
			...
			$dialogEditItem.dialog({
				...
				open: function () {
					$dialogEditItem.html("");
					$.get("admin.php?controller=pjInvoice&action=pjActionEditItem", {
						"id": $(this).data("id")
					}).done(function (data) {
