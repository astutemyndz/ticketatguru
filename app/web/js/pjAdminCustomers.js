	...
	$(function () {
			...
			$frmCreateUser.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "admin.php?controller=pjAdminCustomers&action=pjActionCheckEmail"
					}
				},
				messages: {
					"email": {
			...
			$frmUpdateUser.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "admin.php?controller=pjAdminCustomers&action=pjActionCheckEmail&id=" + $frmUpdateUser.find("input[name='id']").val()
					}
				},
				messages: {
					"email": {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminCustomers&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminCustomers&action=pjActionDeleteUser&id={:id}", beforeShow: onBeforeShow}
				          ],
				...
				paginator: {
					actions: [
					   ...
					   {text: myLabel.revert_status, url: "admin.php?controller=pjAdminCustomers&action=pjActionStatusUser", render: true},
					   {text: myLabel.exported, url: "admin.php?controller=pjAdminCustomers&action=pjActionExportUser", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminCustomers&action=pjActionSaveUser&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminCustomers&action=pjActionGetUser", "name", "ASC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminCustomers&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminCustomers&action=pjActionGetUser");
			});
			return false;
			...
			$grid.datagrid("option", "cache", cache);
			$grid.datagrid("load", "admin.php?controller=pjAdminCustomers&action=pjActionGetUser", "id", "ASC", content.page, content.rowCount);
			return false;
		});
	});
})(jQuery_1_8_2);