...
(function ($, undefined) {
	$(function () {
		"use strict";
		var $frmCreateImageGallery = $("#frmCreateImageGallery"),
			$frmUpdateImageGallery = $("#frmUpdateImageGallery"),
		...
		if ($frmUpdateShow.length > 0 && validate) {
			...
			$frmUpdateShow.validate({
				...
				submitHandler: function(form)
				{
					...
					{
						$.post("admin.php?controller=pjAdminImageGallery&action=pjActionBeforeSave", post_arr[i], callback);
					}
					function callback(data){
						...
						if(loop < post_arr.length)
						{
							...
							$.post("admin.php?controller=pjAdminImageGallery&action=pjActionSave").done(function (data) {
								...
								if(data.code == '101')
								{
									...
									$dialogShowStatus.dialog("option", "close", function () {
							    		...
							    		if(data.code == '200')
							    		{
							    			window.location.href="admin.php?controller=pjAdminImageGallery&action=pjActionShow&id="+data.id+"&err=" + data.text;
							    		}
							    	});
									$dialogShowStatus.dialog("option", "buttons", {
							    		'Close': function () {
		...
		if ($frmCreateImageGallery.length > 0 || $frmUpdateImageGallery.length > 0) {
		...
		if ($frmCreateImageGallery.length > 0 && validate) {
			$frmCreateImageGallery.validate({
				rules:{
					"duration": {
						positiveNumber: true
					}
				...
				submitHandler: function(form){
					...
					$("#frmCreateImageGallery .fdRequired").each(function() {
						if($(this).val() == '')
						{
							valid = false;
					    	$(this).addClass('pj-error-field');
		...
		if ($frmUpdateImageGallery.length > 0 && validate) {
			$frmUpdateImageGallery.validate({
				rules:{
					"duration": {
						positiveNumber: true
					}
			...
			$dialogDeleteShow.dialog({
				...
				buttons: (function () {
					...
					buttons[tbApp.locale.button.delete] = function () {
						...
						$.post("admin.php?controller=pjAdminImageGallery&action=pjActionDeleteShow", {
							"id": id
						}).done(function (data) {
		...
		function formatImage (str, obj) {
			...
			return ['<a href="admin.php?controller=pjAdminImageGallery&action=pjActionUpdate&id=', obj.id ,'"><img src="', src, '" style="width: 80px; display:block;" /></a>'].join("");
		}
		if ($("#grid").length > 0 && datagrid) {
			//console.log(datagrid);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminImageGallery&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminImageGallery&action=pjActionDeleteGallery&id={:id}", beforeShow: onBeforeShow},
				          ],
				columns: [
					{text: myLabel.image, type: "text", sortable: false, editable: false, width: 85, renderer: formatImage},
				...
				dataUrl: "admin.php?controller=pjAdminImageGallery&action=pjActionGetGallery",
				dataType: "json",
				fields: ['gallery_image', 'name', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminImageGallery&action=pjActionDeleteGalleryBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.revert_status, url: "admin.php?controller=pjAdminImageGallery&action=pjActionStatusEvent", render: true},
					   {text: myLabel.exported, url: "admin.php?controller=pjAdminImageGallery&action=pjActionExportEvent", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminImageGallery&action=pjActionSaveGallery&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminImageGallery&action=pjActionGetGallery", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminImageGallery&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminImageGallery&action=pjActionGetGallery");
			});
			return false;
			...
			if(date_time != '')
			{
				...
				$.post("admin.php?controller=pjAdminImageGallery&action=pjActionAddShow&event_id="+event_id+"&period=" + $(this).attr('data-period'), post_data).done(function (data) {
