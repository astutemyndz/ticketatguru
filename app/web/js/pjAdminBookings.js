	...
	$(function () {
		...
		if ($frmCreateBooking.length > 0 && validate) {
			$frmCreateBooking.validate({
				rules: {
					"uuid": {
						required: true,
						remote: "admin.php?controller=pjAdminBookings&action=pjActionCheckUniqueId"
					}
				},
		...
		if ($frmUpdateBooking.length > 0 && validate) {
			$frmUpdateBooking.validate({
				rules: {
					"uuid": {
						required: true,
						remote: "admin.php?controller=pjAdminBookings&action=pjActionCheckUniqueId&id=" + $frmUpdateBooking.find("input[name='id']").val()
					}
				},
				messages: {
					"uuid":{
		...
		if($dialogSelect.length > 0)
		{
			...
			$dialogSelect.dialog({
				...
				open: function (){
					if($('#reload_map').val() == '1')
					{
						$.post("admin.php?controller=pjAdminBookings&action=pjActionGetSeats", $frm.serialize()).done(function (data) {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminBookings&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminBookings&action=pjActionDeleteBooking&id={:id}"},
						 ],
				columns: [
				...
				dataUrl: "admin.php?controller=pjAdminBookings&action=pjActionGetBooking" + pjGrid.queryString,
				dataType: "json",
				fields: ['c_name', 'tickets', 'title', 'date_time', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminBookings&action=pjActionDeleteBookingBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.exported, url: "admin.php?controller=pjAdminBookings&action=pjActionExportBooking", ajax: false}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminBookings&action=pjActionSaveBooking&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		if ($("#grid_invoices").length > 0 && datagrid) 
		{
			...
			function formatId (str) {
				return ['<a href="admin.php?controller=pjInvoice&action=pjActionUpdate&id=', str, '">#', str, '</a>'].join("");
			}
			...
			var $grid_invoices = $("#grid_invoices").datagrid({
				...
				dataUrl: "admin.php?controller=pjInvoice&action=pjActionGetInvoices&q=" + $frmUpdateBooking.find("input[name='uuid']").val(),
				dataType: "json",
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminBookings&action=pjActionGetBooking" + pjGrid.queryString, "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.get("admin.php?controller=pjAdminBookings&action=pjActionGetShows&event_id=" + $(this).val()).done(function (data) {
				$('#boxShow').html(data);
				$('#ticketBox').html("");
				$('#reload_map').val(1);
				$('#seatsBox').hide();
			...
			$('.pj-loader').css('display', 'block');
			$.get("admin.php?controller=pjAdminBookings&action=pjActionGetTickets&event_id=" + $frm.find("select[name='event_id']").val() + "&date_time=" + $(this).val() + "&venue_id=" + $('option:selected', this).attr('data-venue_id') + "&id=" + id).done(function (data) {
					...
					buttons: (function () {
						...
						buttons[tbApp.locale.button.yes] = function () {
							$.ajax({
								...
								dataType: 'json',
								url: "admin.php?controller=pjAdminBookings&action=pjActionSetUseTicket",
