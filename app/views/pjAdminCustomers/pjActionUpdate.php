...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminCustomers&amp;action=pjActionUpdate" method="post" id="frmUpdateUser" class="form pj-form">
		...
		<p>
			...
			<input type="button" value="<?php __('btnCancel'); ?>" class="pj-button" onclick="window.location.href='<?php echo PJ_INSTALL_URL; ?>admin.php?controller=pjAdminCustomers&action=pjActionIndex';" />
		</p>
	</form>
