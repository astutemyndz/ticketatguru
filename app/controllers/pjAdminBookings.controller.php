...
//App::dd($data);
	...
	public function pjActionGetShows()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_GET['event_id']) && count($_GET['event_id']) > 0)
			{
	...
	public function pjActionGetTickets()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_GET['event_id']) && count($_GET['event_id']) > 0 && isset($_GET['venue_id']) && count($_GET['venue_id']) > 0)
			{
				$venue_id = null;
	...
	public function pjActionGetSeats()
	{
		...
		if ($this->isXHR())
		{
			if (isset($_POST['event_id']) && count($_POST['event_id']) > 0)
			{
				$venue_id = null;
				$where = '';
				$pjShowModel = pjShowModel::factory();
