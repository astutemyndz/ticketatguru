...
class pjAdminCustomerRequest extends pjAdmin
{
	...
	// MODEL
	private $pjCustomerRequestModel;
	private $pjMultiLangModel;
	private $pjLocaleModel;
	//private $data = array;

	public function __construct()
	{
		$this->pjCustomerRequestModel = pjCustomerRequestModel::factory();
		$this->pjMultiLangModel = pjMultiLangModel::factory();
		$this->pjLocaleModel = pjLocaleModel::factory();
	}
	
	/***********Ajax Fetching function*************/
	public function pjActionGetCustomerRequest()
	{
		$this->setAjax(true);
	
		if ($this->isXHR())
		{
			$pjCustomerRequestModel = $this->pjCustomerRequestModel
			...
			if (isset($_GET['q']) && !empty($_GET['q']))
			{
				...
				$pjCustomerRequestModel->where('t2.content LIKE', "%$q%");
			}
			if (isset($_GET['status']) && !empty($_GET['status']) && in_array($_GET['status'], array('T', 'F')))
			{
				$pjCustomerRequestModel->where('t1.status', $_GET['status']);
			}
			$column = 'name';
			$direction = 'ASC';
			...
			$total = $pjCustomerRequestModel->findCount()->getData();
			...
			$data = $pjCustomerRequestModel
						->select(" t1.id, t1.content, t1.created_at, t2.first_name, t2.last_name")
						->join('pjCustomer', 't2.id=t1.tk_cbs_customers_id')
						->join('pjArtist', 't3.id=t1.tk_cbs_artists_id')
	...
	public function pjActionIndex()
	{
		...
		$this->appendJs('pjAdminCustomerRequest.js');
	}
