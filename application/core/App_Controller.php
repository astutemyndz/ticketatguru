...
class App_Controller extends CI_Controller
{
	...
	protected function settCookie($array = array(), $XSSFilter  = TRUE)
	{
	...
	{
		if($this->$key) {
			return ($this->getSession($key)) ? TRUE : FALSE;
		}
		return false;
	}
	...
	protected function loadSetFields($force = FALSE, $locale_id = NULL, $fields = NULL)
	{
		...
		{
			self::setFields($locale_id);
	
			# Update session
			if ($registry->is('fields'))
			{
				$this->setSession($fields, $registry->get('fields'));
			}
...
/*
	...
	*/
    ...
    public function set($key, $value)  {  $this->tpl[$key] = $value;  return $this;  } 
